...
| 3 | 岗位发现 | `pf discover` / `pf jobs add` | Kimi 扩叫法；搜索或你手动 | jobs 表 |
| 4 | 团队研究 | `pf research` | Claude（带证据） | teams + signals |
| 5 | 业务背景 | `pf research`（内含） | Claude | why_they_need_this_role、hooks |
| 6 | 找人 | `pf people packet` → `pf people add` | **你**（脉脉/LinkedIn/会议/公众号） | people 表 |
| 7 | 判断人脉价值 | `pf people assess` | Claude | 角色、hook、路径潜力、长期标签 |
| 8 | 评分排序 | `pf rank` / `pf top` | 代码（scoring.yaml） | opportunities + 优先级 |
| 9 | 个性化触达 | `pf outreach <id>` | Claude | 第一条私信 + 2 次 follow-up + 转岗位话术 |
| 10 | 作战卡 | `pf cards` | Claude 写叙述，代码拼卡 | data/cards/*.md |
...
3. **API**：填好 `.env`，`pf run-all`。`PF_SEARCH_PROVIDER=claude_web` 时研究阶段由 Claude 自己联网搜。
...
- **国内模型和 Claude 必须都配吗？** 不必须。缺哪个就按 `models.yaml` 的 `fallback_order` 降级，最后落到 mock 并在 `pf status` 里显示。
