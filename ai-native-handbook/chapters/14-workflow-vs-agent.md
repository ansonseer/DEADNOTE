...
**1. 把可枚举的流程设计成自主 agent。** 你说"帮我做一个自动处理 X 的系统"，它给你一个工具列表 + 一个 while 循环，而 X 其实是五步固定 SOP。识别：让它把执行路径画出来——**如果它能画出一张没有未知分叉的图，那这就该是 workflow**。你该问的是："路径能枚举吗？能的话为什么不写成固定步骤？给出必须让模型决定下一步的具体理由。"
