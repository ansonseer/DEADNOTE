...
1. **搭最小 RAG，分三档加功能。** 用自己的一批文档：第一版只做关键词检索，第二版加向量做混合，第三版加重排。**每一版都用同一个 20~30 条的 golden set 测 recall@5 和端到端正确率**，并单独统计 10 个精确标识符（编号、人名、代号）的命中率。这几组数字会让你以后再不接受"只做向量检索"。
