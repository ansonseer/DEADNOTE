...
你已经知道模型是一个函数（第 10 章），知道它怎么跑、怎么收费（第 12 章）。现在的问题是：函数的**输入**由谁决定。
...
尤其小心**让 AI 自己写回记忆**。写回的东西质量参差、没有过期机制，几个月后记忆文件里堆满当时正确、现在错误的断言。三条纪律：每条记忆带日期和来源；定期人工审阅；**易变的事实不写进记忆，写进"去哪里查"**——「连接参数见 `config/db.ts`」永远不过期，抄一份参数进去三周后就是错的。
