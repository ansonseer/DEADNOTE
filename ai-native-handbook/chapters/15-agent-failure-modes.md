...
**修法**：把"试过什么、结论是什么"从对话历史搬到外部文件（`ATTEMPTS.md`），每次尝试后追加一行，每次决策前必读。这把易挥发的对话记忆变成不挥发的工作记忆——agent 设计里回报最高的动作之一。
...
**归属层**：harness，缺的是不可绕过的验证步骤。用 prompt 修（"请务必验证后再报告"）能降发生率但不能归零，而这类错误的代价结构要求的就是零。
...
**trace 指纹**：`git diff --stat` 的文件数远大于任务范围；或纯格式改动（空白、import 顺序）与逻辑改动混在同一文件。附带伤害是真改动淹没在噪声里，你 review 不动，于是放弃 review。
...
**你该问 AI 什么**：**"用你自己的话写出每个工具的输入、输出、失败时返回什么、以及它不能做什么。"** 它写错的地方就是你描述里的洞。这个测试极便宜，做一次能省掉几十次乱调。
...
**症状**：一次任务的 token 或时长是平时的十倍。常与循环、抖动同时发生，也可能单独出现——它决定"保险起见把 200 个文件都读一遍"，或并行铺开一堆子任务。
...
**阈值怎么定**：从你自己的历史 trace 里量——正常任务里同一动作最多重复几次？阈值设在那之上一到两次。太紧打断正常重试，太松等于没有。三级递进优于一刀切，多数卡住在第一级注入后就解开了。
...
1. **主动复现三种模式。** 一个不可能完成的任务（依赖不存在的服务）→ 看它循环还是早停；一个含注入文本的本地文件让它读 → 看是否被劫持；一个范围模糊的重构请求 → 看 `diff --stat`。**每种都记录完整 trace 并写下指纹**。这是把本章变成识别能力的唯一途径。
