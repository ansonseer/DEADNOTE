...
**归属层**：context。**修法**：中档是 **context 卫生**——工具返回截断与摘要、重读时替换旧快照而非追加、失败尝试折叠成一行结论。强档是**隔离**：把注定产生大量垃圾的探索（扫全仓库、试十种安装方式）丢给 subagent，只回传结论，垃圾留在它自己的 context 里死掉——代价是隔离也切断了共享事实，两个 subagent 可能重复劳动或基于不同快照给出矛盾结论，委派时要把各自需要的事实一次写全（见第 20 章）。同样强的是**外置状态**：重要结论写进文件当作下一段工作的输入，然后开新会话——等于人为做了一次高质量压缩。
...
**同一个根的另一种形态：计划腐烂。** 它在第 3 步列了待办清单，第 20 步世界已经变了——文件被改、某项已被别的改动顺带解决、某项因新事实不再成立——它仍照单执行并打勾。机制与过期快照相同：清单是一次快照，而它默认快照仍有效。指纹是"已勾掉的项与当前代码状态对不上"。修法：清单由代码维护、每轮重读；每完成一项，要求它核对剩余项是否仍成立并给出依据（计划门的设计见第 14 章）。
