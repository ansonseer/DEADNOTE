...
看动作的**归一化形式**（去掉 flag、空白、无关参数）和返回的**前 200 字符**是否重复。两个都重复三次以上，就是它。
...
**你该问 AI 什么**：别问"你为什么一直失败"，那只换来合理化解释。问：**"把最近五步的动作和返回列成表，指出哪两步的返回是等价的；如果等价，说明你在重复什么假设。"** 让它面对结构，而不是解释意图。

### 3. 认知抖动：在两个方案之间横跳

循环的变体。**症状**是它不重复同一动作，却在两三个方案之间来回：改成 A，测试失败，改回 B，失败，又改成 A。diff 来回翻转。
...
**症状**：几十步后，它做的事和你最初要的没关系了。你要"修复登录超时"，四十步后它在优化日志格式，交付时总结"已完成日志系统改进"。
...
**子型 B：调对了工具，用错了语义。** 更隐蔽：用 `search` 做"列出全部"，用模糊匹配做精确校验，对只读工具期待副作用。机制是**描述模糊**——写了"搜索文档"，没写"返回相关度前若干条，不保证召回全部"。模型据此建立了错误的期望，后面所有推理都跑在它上面。修法：工具描述必须包含**它不做什么**和**返回的完整性保证**。
...
**修法（强，且必须做）**：所有工具返回**结构化三段**——状态（ok / error）、内容、失败时的可操作信息（跑在哪个目录、用了什么参数、下一步能试什么）。空结果要显式说"匹配 0 条"，不能返回空串。
...
**归属层**：context。**修法**：中档是 **context 卫生**——工具返回截断与摘要、重读时替换旧快照而非追加、失败尝试折叠成一行结论。强档是**隔离**：把注定产生大量垃圾的探索（扫全仓库、试十种安装方式）丢给 subagent，只回传结论，垃圾留在它自己的 context 里死掉——代价是隔离也切断了共享事实，子 agent 之间可能重复劳动或互相矛盾（见第 20 章）。同样强的是**外置状态**：重要结论写进文件当作下一段工作的输入，然后开新会话——等于人为做了一次高质量压缩。
...
**修法**：后续步骤要用的关键值**当场写进文件或显式状态块**，不要指望它从对话历史里捞——**任何需要它在 10 步之后还记得的东西，都必须落到文件系统上。**
...
**症状**：直接编辑一个从未读过的文件；引用不存在的字段名；假设项目用了某框架的默认目录结构，而它没有。
...
**1. 给合理化的解释，而不是机制。** 你问"刚才为什么一直失败"，它会写出一段流畅自洽的事后叙述——它无法回看当时的采样过程，那段话是新生成的。识别：解释里没有步号和返回原文，就是编的。要求它"用 trace 里的步号和原文支持每个结论"。

**2. 一律推荐"加一条 prompt 规则"。** 识别：修法里没有代码、hook、权限或结构改动时，问它"这条规则失效的概率是多少？失效一次会怎样？有没有确定性的修法？"

**3. 症状层当根因层。** 它修"模型说错话"那一步，不往前找首偏离点。识别：让它明确回答"输入正确但输出错误的第一步是哪一步"，而不是"哪里出问题了"。

**4. 一次改五个地方。** 它同时改 prompt、工具描述、加两个 hook、调 loop，你就不知道哪个起了作用，下次也复现不了。识别：diff 里超过一处根因性改动就打回，要单变量（见第 21 章）。
...
回忆你最常遇到的三种失败（通常是假完成、范围蔓延、循环）。每种写四行：它的 trace 指纹是什么？防护放弱 / 中 / 强哪一档、为什么？具体形式（CLAUDE.md 里的哪一句 / 哪个 hook 拦什么 / 改哪段 loop）？**我要构造一个什么样的输入，让它必须被拦住**？
