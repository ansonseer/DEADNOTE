...
反过来看，一个 skill 如果依赖了没有声明的能力，会怎样：在你的机器上它能跑（因为你的环境恰好有那个命令、那个环境变量、那个已登录的 CLI），换一台机器、换一个人、换一个 agent 会话，它静默失效——模型找不到那个命令，于是"想办法"——用别的方式模拟，或者跳过那一步报告完成。识别方法：让一个**干净会话**（没有你的环境变量、没有你的登录态）跑这个 skill；它卡在哪一步，哪一步就有隐含依赖。
...
治理层面：每个 skill 有 owner、有版本号或变更记录、有"最后验证日期"；目录按领域组织，命名一眼能看出适用范围；团队或客户共享时，划清"通用 skill"与"某项目专用 skill"的边界，专用的不放进通用库。skill 泛滥的征兆是：目录里超过一半的 skill 你说不出上次触发是什么时候——那就该做一次清理：把触发次数为零的归档，把重叠的合并。
...
AI 写 skill 时的错误有很强的规律性，因为它的默认输出形状总是"一大段自然语言指令"。以下每条给一个可观察症状或一句能问 AI 的话。
...
**3. 描述过窄，永远触发不到。** 描述绑定了具体日期、具体文件名、具体人名。症状：触发次数长期为零，而对应任务在发生；模型在没有 skill 的情况下自行发明流程。问 AI："用户会用哪些不同说法描述这类任务？描述覆盖了几种？"

**4. 大量背景知识、没有可执行检查项。** 正文读起来像公司介绍或价值观。症状：加载了 skill 和没加载的输出几乎一样（对照实验一跑就现形）。问 AI："把这个 skill 的每条规则改写成'如果 X 则 Y'，X 可观察、Y 是动作。改不了的标出来。"

**5. 依赖未声明的工具或环境状态。** 症状：在你的机器上能跑，换会话或换人就卡；trace 里出现"command not found"之后模型换了方法继续。识别：用干净会话跑；或问 AI："这个 skill 假设了哪些命令、环境变量、目录、登录态存在？"
...
**11. skill 过期但继续被触发。** 引用的命令、API、目录已变。症状：某 skill 的执行成功率随时间下滑；trace 里固定某一步开始报错然后被绕过。识别：查"最后验证日期"；定期用干净会话跑执行测试。
...
6. **退化测试。** 在已有一批 skill 的项目里新增一个 skill 后，重跑一批无关任务和相邻 skill 的样本，统计有没有被新 skill 吸走触发、相邻任务质量有没有下降。这是大多数人从不做、而 skill 泛滥的系统最需要的测试。
