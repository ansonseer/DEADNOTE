...
解析：
- 层：data 层的症状（pool 耗尽），根因在 service 层（导出接口在事务里做了外部 HTTP 调用，或者查询完没归还连接）。DB CPU 20% 直接否定"数据库负载过高"——这是 AI 把症状层当根因，把假设当事实。
...
- 修复：幂等键（`order_id + email_type`）落库后再发送，发送成功再 ack；处理时长长的任务用心跳延长可见性。
...
每次注入配一份**录制/回放**：把当次的 trace、日志切片、context dump 存进 `lab/incidents/NN/`。三个月后它们是你的案例库、内容素材、面试作品。
