...
**症状与鉴别**：浏览器 console 出现 `has been blocked by CORS policy`，network 面板里那个 `OPTIONS` 的状态码通常是真凶，而同一个 URL 用 `curl` 打完全正常。记住这一对判据：
...
Webhook 是"反过来的 API 调用"：对方会因超时而**重复投递**，所以 handler 必须幂等（拿事件 ID 做自然键），且必须**先返回 2xx 再异步干活**——在 handler 里做重活会让对方判定失败、再投一遍。
