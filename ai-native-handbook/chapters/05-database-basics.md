...
这一章给你三层能力：读懂一份 schema 在断言什么、预测一条查询在数据量涨一百倍后会怎样、推演两个请求同时到达会发生什么。数据库在真实负载下怎么坏见第 06 章；这一章讲那些故障的**根**。
...
| 随机 UUID | 全局唯一、客户端可预生成、不泄漏信息 | 更宽、随机分布导致索引写入分散到各处，缓存命中率下降 |
...
**时间必须带时区，且存 UTC。** 存无时区本地时间的后果：夏令时切换时某个时刻可能不存在或出现两次；服务器换区域后全部数据偏移几小时。规则：**存储用带时区的时间戳（内部即 UTC），只在展示层转成用户时区**。但"用户选的会议日期"这种**未来的本地时间**是另一回事：时区规则本身会被政府改，存成 UTC 之后会指向错误的本地时刻，应存"本地时间 + 时区标识"。**问 AI**："这列是绝对时刻还是用户本地日历时间？夏令时切换那天会怎样？"

**别用字符串存日期、数字、布尔。** 文本日期勉强还能排序，`'5'` 和 `'10'` 就错了；而且数据库无法用类型检查挡住 `'not-a-date'`。
...
第二行的"幻读"格要小心：名字一样，各引擎给的东西不一样。多数实现的 Repeatable Read 实际是**快照隔离**——整个事务读的是某一时刻的一致快照，别人事后插入的行你根本看不见，所以传统意义的幻读不出现。但 SQL 标准并不禁止 Repeatable Read 出现幻读。稳定的用法：**隔离级别的名字是行为的下限承诺，不是实现的描述**；不要按名字推断你得到了什么保证，要按"我这段逻辑依赖的读，在我提交之前会不会失效"来推。
...
- **Read Committed**：两个事务都读到 10，第二条 UPDATE 覆盖第一条，卖出 16 件。丢失更新，全程无报错。
- **快照隔离（Repeatable Read）**：结果**取决于引擎**。有的会检测到"你要改的这一行，在你的快照建立之后被别人改过"，直接让第二个事务中止并报序列化冲突；有的则让它按最新值继续，退回到丢失更新。前一种不算错，但它把问题变成了**你必须写重试逻辑**——AI 生成的代码几乎从不带重试，于是并发一高就是一片 500，而且报错发生在数据库层、离业务代码很远，很难追。
- **Serializable**：能挡住，代价是中止率更高，同样必须重试。

所以正确的结论不是"调隔离级别"，而是：**"读—判断—写"（read-modify-write）这个形状本身要改掉。** AI 写出来的库存、计数、余额代码几乎全是这个形状。
...
**写偏斜：为什么它比超卖更难防。** 超卖里两个事务抢的是同一行，所以还有"同一行被改过"这个可检测的信号。写偏斜连这个信号都没有：值班表里两个医生，规则是"任何时刻至少有一人在岗"。两人同时请假，各自的事务先读到"在岗人数 = 2"，各自判断"减掉我还剩 1，合法"，然后各自 UPDATE **自己那一行**。两个事务改的不是同一行，快照隔离找不到任何冲突，两笔写入都合法提交，结果无人在岗。

凡是"读一批行 → 判断 → 写另一批行"的不变量都是这个形状，而它落在所有常规工具的缝隙里：CHECK 只管一行内部（5.3），行锁锁的是你要改的行、而不是你判断依据所在的行。两条出路：把判断落到一个**能被锁住的物理位置**上（比如为每个班次建一行，请假时更新它并让数据库做判断，把跨行不变量压成单行不变量），或者上 Serializable 并写好重试。识别方法：**问自己"我的判断依据，是不是来自我不打算修改的那些行"**——是，就有写偏斜风险。
...
| `SELECT *` 拉全表到内存过滤 | 内存随数据量增长、进程被 OOM 杀掉；耗时与总行数成正比 | "过滤发生在数据库里还是应用里？" |
