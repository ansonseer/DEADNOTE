...
**规划/决策层。** 从"我在哪"和"目标是什么"推出动作序列：路径规划、抓取点选择、任务分解。这是离 LLM 最近的一层——VLA 类模型（第 7 节）主要活在这里。失效方式：目标不可达却不报错、路径几何上可行但动力学上不可行（转弯半径太小）、规划太慢跟不上世界变化。可观测信号：规划输出（路径点、目标姿态）和规划耗时。
...
在 harness 语言里（第 18 章）：permission、确认门、熔断器在硬件里是**生命级**的，实现必须从"模型看到的规则"下沉到"模型碰不到的电路和固件"。一条写在 system prompt 里的"不要超过 5 牛顿"不是安全措施，一个设在驱动器里的电流上限才是。问 AI 的话："**如果规划器进程被 kill -9，哪一层保证电机停下？给我那一层的代码或接线。**"答不出来就是没有安全层。
...
**真实数据为什么不可替代。** 仿真只能模拟你知道的物理，不知道的（那块橡胶垫的摩擦、那台电机第三个齿轮的齿隙）只有现实能告诉你。成熟做法：仿真做粗训练和大规模 eval，现实做微调和验收，现实里发现的差距反哺进仿真参数。
...
- **自主采集**：机器人自己试，记录结果。可无人值守，但成功率低时大部分是失败样本（对强化学习有用，对模仿学习几乎没用），且有安全和磨损成本。
...
**物理正确性。** 生成的画面看起来像真的，不等于物理上对：杯子穿过桌面、物体凭空消失、能量凭空增加。看起来合理和物理正确之间的差距，就是"用它规划"和"用它做视频"的差距。规划要的是可信的因果，不是漂亮的像素。
...
3. **一致性的代价**：LLM 长文本前后矛盾，读者困惑；world model 长时程预测偏离，规划出一条撞墙的路径。
...
**harness 上的判断力可以整套迁移**，只要每一格补上物理量纲——时长、力、概率、安全等级。让 AI 设计具身 harness 时问它："**每个动作原语的最坏执行时长、失败后的世界状态、以及模型之下的确定性校验，各是什么？**"三项缺一，这个 harness 就是软件 agent 的直译。
...
- **状态机思维**（第 14 章）：任务流程就是状态机，每个状态的进入、退出、超时转移都要显式——"隐式状态"在这里的代价是机器卡在半空中。
...
**入门路径**：先做感知 + 简单控制（读传感器、估计状态、到点或平衡），再上学习方法（示范学习、策略训练），最后碰 world model。顺序不能反：没有分层的手感，学习方法出问题时无法定位层级。
...
**8. 推荐昂贵硬件、过早深入某个框架，或建议止于"学 ROS"。** 症状：入门建议第一条是买机械臂、第二条是装某框架全家桶。识别：问"**这项投入在我换硬件或框架时还剩多少？训练的是哪项可迁移能力？**"答"熟悉这个框架"就是没答。
...
**练习 3：仿真 95% 现实失败的差距清单。** 一个抓取策略仿真成功率 95%，现实 40%。列出至少八个可能的差距来源并归入第 4 节的五类。评判标准：五类每类至少一条，标定类必须含手眼标定，感知类必须含反光/透明物体。再写出你会先查哪一类、看什么信号——正确顺序通常是先标定和感知（最便宜、最常见），最后才是动力学。
...
**练习 2：真实闭环 + 数据手册纪律。** 买一块入门开发板、一个传感器（距离或角度）、一个执行器（舵机或小电机），让 AI 给出接线方案和代码。上电之前，对每一个引脚、电压、电流、通信参数逐项查数据手册，在一张表里记录"AI 说的 / 手册说的 / 是否一致"。然后做一个到点或跟随的闭环，把仿真里调好的 PID 参数直接搬过来，记录它在现实里的曲线差异——这就是你的第一份 sim2real 记录。
...
**练习 4：最小物理项目。** 开发板 + 摄像头 + 让一个视觉模型描述画面并据此做一个动作（亮灯、转向）。记录三件事：画面到动作的端到端延迟、模型被光照和角度欺骗的次数、你为防止危险动作加了哪些确定性守卫。用第 22 章的格式写一篇诊断记录，这是你第一篇具身方向的内容。
