...
状态 s 可以是低维的（关节角、位置、速度），也可以是高维的（图像）。高维时通常先学一个编码器把图像压成潜变量（latent），再在潜空间里学动力学——因为像素级预测太浪费，且大部分像素和决策无关。当前最引人注目的形态是**生成式视频/物理模型**：输入当前画面和动作，生成接下来的画面。这确实是 world model，但有两个核心难题：
...
VLA（视觉-语言-动作）类模型的角色是：看图像、读任务描述、输出动作。把它放回第 2 节的分层图里，它主要占的是**规划/决策层**，向下接一个确定性控制器，向上接任务指令。用第 18 章的 harness 语言翻译，对应关系几乎一一映射：
...
| permission / 确认门 | 安全守卫：力矩上限、工作空间围栏、人体检测 | 生命级；必须独立于模型进程 |
...
这张表说明：**harness 上的判断力可以整套迁移**，只要每一格补上物理量纲——时长、力、概率、安全等级。VLA 的边界也由此清晰：它擅长语义层（这是杯子、要放进碗里、先挪开挡着的东西），不擅长也不应该负责毫秒级的力控。让 AI 设计具身 harness 时问它："**每个动作原语的最坏执行时长、失败后的世界状态、以及模型之下的确定性校验，各是什么？**"三项缺一，这个 harness 就是软件 agent 的直译。
...
- **可观测性**（第 8、21 章）：每层信号都要能记录、回放、画曲线。机器人圈的"rosbag/日志回放"和你的 trace 是同一个东西。
- **eval 设计**（第 21 章）：分桶、覆盖长尾、区分"任务成功"与"过程安全"。
- **安全门与 harness 思维**（第 18 章）：换了实现层级，思路不变。
- **训练机制的直觉**（第 11 章）：RLHF/RLVR ↔ 机器人策略学习——奖励设计、奖励作弊（reward hacking，仿真里机器人学会抖动刷分）、离线数据与在线交互的取舍，全是同一批问题。
- **对抗面**（第 9 章）：prompt injection ↔ 传感器欺骗（贴一张图片让视觉系统认错、激光雷达被强光致盲）。原则相同：不可信输入不能直接驱动不可逆动作。
- **scaling 的直觉**（第 11 章）：瓶颈从算力换成真实交互数据，曲线的形状和"数据质量 > 数量"的判断照旧。
...
- **实时系统**：确定性调度、抢占、jitter、为什么 Python 主循环不能跑关键控制。
- **安全标准的存在**：工业协作机器人有力和速度的限制规范、电气安全有等级划分——你不必背条款，但要知道它们存在、在客户现场是硬门槛。
- **机械与电气常识**：扭矩够不够、线径够不够、发热怎么散、螺丝会松、连接器会氧化。

**硬件项目的工程现实**（AI 最不理解的部分）：迭代周期以周计而非以分钟计——一个零件要采购、要等快递、装错了再等；调试靠仪器而非日志——万用表、逻辑分析仪、示波器看到的东西日志里没有；成本结构不同——软件的边际成本趋零，硬件每次实验都消耗物料、磨损和人的看管时间；返工不可逆——烧掉的板子不能 git revert。规划硬件项目时，把每一步的等待时间和报废概率写进计划，否则它是软件化的幻想。
...
- **特定框架**：最后学，因为它们换得最快，而且有了前四项之后学任何框架都是一两周的事。

**入门路径**：先做感知 + 简单控制（读传感器、估计状态、到点或平衡），再上学习方法（示范学习、策略训练），最后碰 world model。顺序不能反：没有分层的手感，学习方法出问题时你无法定位层级。

**从软件侧切入，而非硬件制造。** 机器人公司里最缺、且你现在的能力最能迁移的岗位在软件侧：数据管线（遥操作数据的采集、对齐、标注、质检）、eval 基础设施（仿真评估、真实成功率统计、分桶回归）、仿真工具链、运营侧的可观测性。这些是纯软件工程，但每条数据都贵，杠杆比 LLM 公司同类岗位更大。
...
**转向信号**：客户需求指向物理（"能不能让它自己去拿"）、内容受众对具身话题的反馈明显更强、你在仿真和硬件小项目里持续兴奋而不是持续挫败。三条同时出现再加码，否则继续在软件侧积累。

### 10. 与终极目的的连接

你的终极目的是"解放和集中人类思想"。在这一章里它有两个具体落点。具身 AI 是**解放劳动**的直接手段：把人从重复的、伤身体的物理操作中解放出来，比把人从写邮件中解放出来更直接地触及"人的时间用在哪"。world model 是**理解与预测**的工具：一个能回答"如果这样做，世界会怎样"的模型，是把人类对因果的直觉外化成可查询、可验证的资产——这和你在 DEAD NOTE 里做的"把认知外化"是同一个动作，只是对象从思想换成了物理。判断技术选择时用这两个落点做标尺：让人更少被物理劳动占用、或让因果更可预测的方案，就在你的路线上。
...
**3. 忽略安全停止与物理边界，或把安全层写在模型层。** 症状：安全规则出现在 prompt 或应用代码的 `if` 里，没有硬件急停、没有看门狗、关节限位靠软件。识别：问"**规划器进程被 kill -9 时，哪一层让电机停？**"以及"**安全层的输出是确定性的吗——同样输入永远同样输出？**"任何一个安全动作经过了模型推理，就不是安全层。

**4. 建议用 LLM 做低层控制。** 症状：架构图里 LLM 直接输出关节角或电机 PWM；控制周期依赖网络往返。识别：问"**这一层的截止时间是多少毫秒，你的推理延迟的最坏值是多少？**"两个数放在一起就见分晓。

**5. 仿真里验证就宣称现实可用。** 症状："在仿真中达到 95% 成功率，可以部署"。识别：要求它按第 4 节的五类差距（动力学、感知、时序、标定、分布）逐项说明仿真建模了什么、没建模什么，以及现实验收的样本量和分桶。

**6. 低估数据采集的成本和时间。** 症状：计划里写"收集 10 万条示范"却没有人时、设备、标注、质检的预算；把仿真数据和真实数据当同一种东西算总量。识别：问"**一小时遥操作产出多少条可用数据、需要几个人、失败样本占多少、恢复样本从哪来？**"

**7. 把 world model 讲成"另一个 LLM"。** 症状：只谈"预测下一帧"，不谈误差随时程的增长、不谈物理约束怎么保证。识别：第 6 节末尾的四个问题。

**8. 推荐昂贵硬件、过早深入某个框架，或建议只停留在"学 ROS"。** 症状：入门建议第一条是买几千块的机械臂、第二条是装某个框架全家桶。识别：问"**这项投入在我换硬件或换框架时还剩多少？**"以及"**它训练的是哪一项可迁移能力？**"答案是"熟悉这个框架"就是没答。

**9. 硬件项目规划过于软件化。** 症状：计划以"天"为单位、没有采购等待、没有报废预算、没有"用仪器验证"的步骤。识别：让它把每一步的等待时间和不可逆点标出来；标不出来的计划不可信。
...
- 上层进程崩溃时，哪一层让系统进入安全状态？验证过（真的 kill 过）吗？
