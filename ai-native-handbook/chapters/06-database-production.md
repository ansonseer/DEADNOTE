...
**serverless 环境的连接爆炸**是极端形态：每个实例有自己的池，实例数由平台伸缩，乘数你控制不了。解法是在数据库前放共享池，让应用侧池大小趋近 1。
...
**向量索引在生产的特殊问题**（检索链路见第 16 章）：近似最近邻索引要把大量结构常驻内存，内存不够时性能断崖式下跌；大批量增删后召回退化、需重建。最致命的是 **embedding 版本漂移**——换了嵌入模型后新旧向量在同一索引里根本不可比，检索结果**静默变差而不报错**。所以向量必须带版本标记，换模型 = 全量重算，要当成一次迁移来规划。
