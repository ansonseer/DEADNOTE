...
说"不"的那一刻，你从供应商变成顾问：能拿到项目的人很多，敢说这个项目不该做的人很少，而后者才会被叫回去做第二个。

### 13. 基线：不在第一周量，就永远量不到

价值度量的机制简单而残酷：**基线只存在于系统上线之前**。上线之后你再想知道"以前要多久"，只能靠人回忆，而人的回忆会被新系统的存在污染——他们会记得最糟的那次，或者反过来美化过去。所以第一周的产出里必须有三个带取数方式的数字：

- **周期时间**：从单据第一个时间戳到最后一个，取分布的中位数和 P90——只取平均会被少数极端单子拉飞。
- **错误率与返工率**：怎么定义一次"错"？谁发现的？在哪一步发现？返工一次动几个人？
- **单位成本**：一单占多少人时，按什么工资带折算。

这三个数字要写进报告并让客户确认。确认不是形式，是防止半年后有人说"我们本来也没那么慢"。

价值汇报最常见的塌方是用虚荣指标替代真指标：

| 虚荣指标（好看，无意义） | 对应的真指标 |
|---|---|
| 调用次数、生成条数 | 输出被**采纳**的比例（人改了多少才用） |
| 用户满意度评分 | 端到端周期时间的变化 |
| "节省了多少小时" | 省下的小时具体去做了什么、产出变了没有 |
| 覆盖率、上线模块数 | 瓶颈那一步的队列长度 |

归因要提前设计，否则成果会被季节性吞掉。最可靠的是**同期对照**：一半单子走新流程、一半走旧流程，同期比较；做不到分流就用同比加单量归一化，并在报告里写明你排除不掉的干扰。**主动写出自己度量的弱点，比被别人指出来强得多。**
...
自检判据只有一条：**给一个没参加过 discovery 的人读，他能不能复述出你为什么推荐这个、为什么不推荐那个**。复述不出来，说明推理还在你脑子里。

## AI 在这里最常犯的错，以及怎么识别

Discovery 是 AI 最不可靠的场景：它没有现场，却极擅长生成看起来像现场的文字。下面每一条都配了识别方法和一句你可以直接问的话。
...
| 集成代码假设 API 存在且稳定 | 代码里出现没人给过文档的 endpoint，字段名恰好符合命名规范 | "列出这段代码依赖的每个外部接口与字段，标注我是否提供过证据。" |
| 安全材料泛泛生成，过不了审 | 数据流图上没有具体系统名、数据分类、跨境标注 | 让它先问你二十个问题再动笔，材料逐条对应客户的审查清单 |
| 价值汇报是形容词 | "显著提升""大幅缩短"，但没有基线数字 | "把每个形容词换成一个数字，数字没有的地方写'未测量'。" |
| 直接给标准架构方案 | 没聊业务就出现完整技术栈方案 | 最强的信号：你给的上下文里业务事实太少，回去补 discovery |

有一条元规律值得记住：**AI 的输出流畅度和它掌握的事实量无关**。在别的章节里，输出错了通常还伴随某种破绽；在 discovery 里，编造出来的公司画像读起来往往比真实的更整洁——因为真实的公司总是充满矛盾和例外。**读到一份"太顺"的 discovery 总结，先怀疑它，而不是庆祝它。**
...
用五个维度读一遍你上班的公司，写一页纸 discovery 报告。你有外部顾问拿不到的信息：谁和谁不对付、哪张报表没人信。评判标准：报告里有没有至少三条"你知道但官方叙述里不会写"的事实；一条都没有，说明你写的是官方叙述。
